...
\hypertarget{idyn_one_chain_tutorial}{}\label{idyn_one_chain_tutorial}\index{Computation of torques in a single chain, using iDyn@{Computation of torques in a single chain, using iDyn}}
In this tutorial we show how to compute the joint torques in a single kinematic chain, using the i\+Dyn library.

Before you read this tutorial you should at least get accustomed with the i\+Dyn library, and the basic of Newton-\/\+Euler algorithm (reading the \doxysectlink{idyn_introduction}{short introduction to i\+Dyn}{0} might be useful).

Remember to include YARP and \doxylink{namespaceiCub}{i\+Cub} as packages, and to link i\+Dyn library in CMake\+Lists.\+txt\+: 
\begin{DoxyCode}{0}
...
\end{DoxyCode}
 note that in this case we need to look for the packages because we are developing a tutorial which is outside \doxylink{namespaceiCub}{i\+Cub} project.
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\mbox{\hyperlink{classiCub_1_1iDyn_1_1iCubArmDyn}{iCubArmDyn}}\ armTorsoDyn(\textcolor{stringliteral}{"{}right"{}});}
...
We can now start using i\+Dyn. In order to perform the kinematic and wrench computation in a chain, we must first tell the limb that we want to use the i\+Dyn\+::\+Recursive\+Newton\+Euler library, by calling a "{}prepare"{} method. In fact, before using our method, a new chain, parallel to the i\+Dyn\+Chain, is built\+: it is a One\+Chain\+Newton\+Euler, and it is made of One\+Link\+Newton\+Euler virtual links, each having its "{}real"{} corresponding i\+Dyn\+Link; in addition, a Base\+Link and a Final\+Link elements are added to the chain, defining the entry points for the kinematic and wrench phases of Newton-\/\+Euler computations.
...
\item DYNAMIC\+: we need q,dq,ddq (joints position, velocity, acceleration) -\/-\/\texorpdfstring{$>$}{>} default
\end{DoxyItemize}


\begin{DoxyCode}{0}
\DoxyCodeLine{armTorsoDyn.prepareNewtonEuler(DYNAMIC);}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{icub-\/tutorials/src/iDyn/oneChainDynamics/\mbox{\hyperlink{icub-tutorials_2src_2anyRobotCartesianInterface_2src_2fakeRobot_2main_8cpp_ae66f6b31b5ad750f1fe042a706a4e3d4}{main}}.cpp}
