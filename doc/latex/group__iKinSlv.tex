...
Classes for on-\/line solution of inverse kinematic of \mbox{\hyperlink{namespaceiCub}{i\+Cub}} limbs based on \href{http://wiki.icub.org/wiki/Installing_IPOPT}{\texttt{ Ip\+Opt}}. The solvers run as on-\/line daemons which connect to the robot to retrieve information on the current joints configuration (along with the bounds) and by requesting a desired pose with queries made through Y\+A\+RP ports they return the corresponding target joints.
...
{\bfseries{q}} property\+: contains the joints configuration which achieves x (D\+O\+F-\/components vector in degrees).
