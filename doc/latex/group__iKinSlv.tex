\doxysubsubsubsection{i\+Kin\+Slv}
\hypertarget{group__iKinSlv}{}\label{group__iKinSlv}\index{iKinSlv@{iKinSlv}}


Classes for on-\/line solution of inverse kinematic of \doxylink{namespaceiCub}{i\+Cub} limbs based on IPOPT.  
...
\[
   \begin{array}{c}
     ...
     \text{s.t.}
     \begin{cases}
       \left\|\mathbf{x}_d-\mathit{K_x}\left(\mathbf{q}\right)\right\|^2=0 \\
       \mathbf{q}_L<\mathbf{q}<\mathbf{q}_U
     \end{cases}
   \end{array}.
   \]

Where the solution $ \mathbf{q} $ is the joints vector with n components (depending on the task and the current dof configuration) that is guaranteed to be found within the physical bounds expressed by $ \mathbf{q}_L $ and $ \mathbf{q}_U $; $
\mathbf{x}_d\equiv\left(x,y,d\right)_d $ is the positional part of the desired pose, $ \mathbf{\alpha}_d $ is the desired orientation and $ \mathit{K_x} $ and $ \mathit{K_{\alpha}} $ are the forward kinematic maps for the position and orientation part, respectively; $ \mathbf{q}_r $ is used to keep the solution as close as possible to a given rest position in the joint space (weighting with a positive factor $ \mathit{w}
< 1 $ and also through the diagonal matrix $ \mathit{W}_r $ which allows selecting a specific weight for each joint).\hypertarget{group__iKinSlv_protocol_sec}{}\doxysubsubsubsubsection{\texorpdfstring{Solver protocol}{Solver protocol}}\label{group__iKinSlv_protocol_sec}
...
{\bfseries{dof}} request\+: example (\mbox{[}dof\mbox{]} (1 1 0 1 ...)), specifies which dof of the chain are actuated (by putting 1 in the corresponding position) and which not (with 0). The length of the provided list of 1\textquotesingle{}s and 0\textquotesingle{}s should match the number of chain\textquotesingle{}s dof. The special value 2 is used to keep the link status unchanged and proceed to the next link.

{\bfseries{resp}} request\+: example (\mbox{[}resp\mbox{]} (20.\+0 0.\+0 0.\+0 ...)), specifies for each joint the rest position in degrees. The reply will contain the rest position resulting from the application of joints limits.

{\bfseries{resw}} request\+: example (\mbox{[}resw\mbox{]} (1.\+0 0.\+0 1.\+0 ...)), specifies for each joint the weight used to compute the rest position minimization task. The reply will contain the weights as result of a saturation over zero.
...
\begin{DoxyNote}{Note}
One single command sent to the streaming input port can contain multiple requests\+: e.\+g. (\mbox{[}pose\mbox{]} \mbox{[}xyz\mbox{]}) (\mbox{[}dof\mbox{]} (1 0 1 2 1)) (\mbox{[}xd\mbox{]} (-\/0.\+1 0.\+1 0.\+1) (\mbox{[}tok\mbox{]} 1.\+234) ...)
...
{\bfseries{dof}} request\+: example \mbox{[}set\mbox{]} \mbox{[}dof\mbox{]} (1 2 1 0 ...), \mbox{[}get\mbox{]} \mbox{[}dof\mbox{]}. The reply will contain the current dof as result of the reconfiguration. The result may differ from the request since on certain limb (e.\+g. arm) some links are considered to form a unique super-\/link (e.\+g. the shoulder) whose components cannot be actuated separately.

{\bfseries{resp}} request\+: example \mbox{[}set\mbox{]} \mbox{[}resp\mbox{]} (20.\+0 0.\+0 0.\+0 ...), \mbox{[}get\mbox{]} \mbox{[}resp\mbox{]}. Set/get for each joint the rest position in degrees. The reply will contain the rest position resulting from the application of joints limits (lim request does affect the range).

{\bfseries{resw}} request\+: example \mbox{[}set\mbox{]} \mbox{[}resw\mbox{]} (1.\+0 0.\+0 1.\+0 ...), \mbox{[}get\mbox{]} \mbox{[}resw\mbox{]}. Set/get for each joint the weight used to compute the rest position minimization task. The reply will contain the weights as result of a saturation over zero.
...
{\bfseries{conv}} request\+: example \mbox{[}set\mbox{]} \mbox{[}conv\mbox{]} ((max\+\_\+iter 200) (tol 0.\+001)), \mbox{[}get\mbox{]} \mbox{[}conv\mbox{]}. Set/get the options for specifying solver\textquotesingle{}s convergence.

Commands issued through the \mbox{[}ask\mbox{]} vocab\+:

{\bfseries{xd}} request\+: example \mbox{[}ask\mbox{]} (\mbox{[}xd\mbox{]} (x y z ax ay az theta)) (\mbox{[}pose\mbox{]} \mbox{[}xyz\mbox{]}) (\mbox{[}q\mbox{]} (...)). Ask to solve for the target xd. User can specifies a different starting joint configuration q and the pose mode. The reply will contain something like \mbox{[}ack\mbox{]} (\mbox{[}q\mbox{]} (...)) (\mbox{[}x\mbox{]} (...)), where the found configuration q is returned as well as the final attained pose x.
...
{\bfseries{status}} request\+: example \mbox{[}status\mbox{]}, returns \mbox{[}ack\mbox{]} "{}not\+\_\+started"{}\texorpdfstring{$\vert$}{|}"{}running"{}\texorpdfstring{$\vert$}{|}"{}suspended"{}.
...
{\bfseries{ /$<$solver\+Name$>$/out }} streams out a bottle containing the result of optimization instance. The format is (\mbox{[}xd\mbox{]} (...)) (\mbox{[}x\mbox{]} (...)) (\mbox{[}q\mbox{]} (...) (\mbox{[}tok\mbox{]} ...)) as following\+:
