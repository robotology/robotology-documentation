\doxysubsubsection{objects\+Properties\+Collector}
\hypertarget{group__objectsPropertiesCollector}{}\label{group__objectsPropertiesCollector}\index{objectsPropertiesCollector@{objectsPropertiesCollector}}
...
Provides a on-\/line yarp-\/oriented database to collect properties such as positions, colors, shapes, grasping points and so on about objects that are of interest for your specific application, i.\+e. normally real objects that the robot can play with but ideally any kind of objects you can think of. The user can set, get, add, remove items and make queries to the database. ~\newline
 Importantly, the module is capable of running in real-\/time.\hypertarget{group__objectsPropertiesCollector_proto_sec}{}\doxysubsubsubsection{\texorpdfstring{Protocol}{Protocol}}\label{group__objectsPropertiesCollector_proto_sec}
Notation used hereafter to explain available commands\+: \mbox{[}.\mbox{]} is a Vocab, "{}."{} is a string, \texorpdfstring{$<$}{<}.\texorpdfstring{$>$}{>} is a Value (i.\+e. string, double, int) or a List (so that also complex properties such as images can be attached to the objects).
...
{\bfseries{add}} ~\newline
{\itshape Format}\+: \mbox{[}add\mbox{]} (("{}prop0"{} $<$val0$>$) ("{}prop1"{} $<$val1$>$) ...) ~\newline
{\itshape Reply}\+: \mbox{[}nack\mbox{]}; \mbox{[}ack\mbox{]} ("{}id"{} $<$num$>$) ~\newline
...
{\bfseries{del}} ~\newline
{\itshape Format}\+: \mbox{[}del\mbox{]} (("{}id"{} $<$num$>$) (prop\+Set ("{}prop0"{} "{}prop1"{} ...))) ~\newline
...
The special command "{}\mbox{[}del\mbox{]} (all)"{} clears the current content of the database.

{\bfseries{get}} ~\newline
{\itshape Format}\+: \mbox{[}get\mbox{]} (("{}id"{} $<$num$>$) (prop\+Set ("{}prop0"{} "{}prop1"{} ...))) ~\newline
{\itshape Reply}\+: \mbox{[}nack\mbox{]}; \mbox{[}ack\mbox{]} (("{}prop0"{} $<$val0$>$) ("{}prop1"{} $<$val1$>$) ...) ~\newline
 {\itshape Action}\+: return the required properties assigned to the stored item. ~\newline
 The special command "{}\mbox{[}get\mbox{]} ((id $<$num$>$))"{} returns all the properties.

{\bfseries{set}} ~\newline
{\itshape Format}\+: \mbox{[}set\mbox{]} (("{}id"{} $<$num$>$) ("{}prop0"{} $<$val0$>$) ...) ~\newline
 {\itshape Reply}\+: \mbox{[}nack\mbox{]}; \mbox{[}ack\mbox{]} ~\newline
 {\itshape Action}\+: add/modify properties of the stored item.

{\bfseries{lock}} ~\newline
{\itshape Format}\+: \mbox{[}lock\mbox{]} (("{}id"{} $<$num$>$)) ~\newline
 {\itshape Reply}\+: \mbox{[}nack\mbox{]}; \mbox{[}ack\mbox{]} ~\newline
...
{\bfseries{unlock}} ~\newline
{\itshape Format}\+: \mbox{[}unlock\mbox{]} (("{}id"{} $<$num$>$)) ~\newline
 {\itshape Reply}\+: \mbox{[}nack\mbox{]}; \mbox{[}ack\mbox{]} ~\newline
 {\itshape Action}\+: unlock the specified item.

{\bfseries{owner}} ~\newline
{\itshape Format}\+: \mbox{[}owner\mbox{]} (("{}id"{} $<$num$>$)) ~\newline
 {\itshape Reply}\+: \mbox{[}nack\mbox{]}; \mbox{[}ack\mbox{]} ("{}owner\+\_\+name"{}) ~\newline
 {\itshape Action}\+: ask for the port name of the item owner; a name equal to "{}all"{} means that the item is not locked by any agent.

{\bfseries{time}} ~\newline
{\itshape Format}\+: \mbox{[}time\mbox{]} (("{}id"{} $<$num$>$)) ~\newline
...
{\bfseries{ask}} ~\newline
{\itshape Format}\+: \mbox{[}ask\mbox{]} (("{}prop0"{} "{}$<$"{} $<$val0$>$) \texorpdfstring{$\vert$}{|}\texorpdfstring{$\vert$}{|} ("{}prop1"{} "{}$>$="{} $<$val1$>$) ...) ~\newline
 {\itshape Reply}\+: \mbox{[}nack\mbox{]}; \mbox{[}ack\mbox{]} ("{}id"{} ($<$num0$>$ $<$num1$>$ ...)) ~\newline
 {\itshape Action}\+: query the database to find all the items whose properties match the conditions given in the command. You can compose multiple conditions using the boolean operators such as "{}\texorpdfstring{$\vert$}{|}\texorpdfstring{$\vert$}{|}"{} for {\itshape or} and "{}\&\&"{} for {\itshape and} and each condition has to be expressed giving the property name, the value to compare with and the corresponding relational operator (e.\+g. "{}$>$"{}, "{}$<$="{}, "{}=="{}, ...).~\newline
 Commands such as "{}\mbox{[}ask\mbox{]} ((prop0) \texorpdfstring{$\vert$}{|}\texorpdfstring{$\vert$}{|} (prop1))"{} will query whether the properties exist or not. ~\newline
 The special command "{}\mbox{[}ask\mbox{]}  (all)"{} returns the whole set of ids present within the database. ~\newline
 In order to simplify the implementation, nested conditions such as (cond1) \&\& ((cond2) \texorpdfstring{$\vert$}{|}\texorpdfstring{$\vert$}{|} (cond3)) are not handled; however, this is not a real limitation since nested conditions can be properly expanded\+: indeed, the previous example can be cast back to (cond1)\&\&(cond2) \texorpdfstring{$\vert$}{|}\texorpdfstring{$\vert$}{|} (cond1)\&\&(cond3).

{\bfseries{quit}} ~\newline
 {\itshape Format}\+: \mbox{[}quit\mbox{]} ~\newline
 {\itshape Reply}\+: \mbox{[}ack\mbox{]} ~\newline
 {\itshape Action}\+: quit the module.\hypertarget{group__tutorial__perceptiveModels_lib_sec}{}\doxysubsubsubsection{\texorpdfstring{Libraries}{Libraries}}\label{group__tutorial__perceptiveModels_lib_sec}
...
Linux and Windows.\hypertarget{group__objectsPropertiesCollector_reqexample_sec}{}\doxysubsubsubsection{\texorpdfstring{Examples}{Examples}}\label{group__objectsPropertiesCollector_reqexample_sec}
...
\DoxyCodeLine{\ }
\DoxyCodeLine{command:\ [set]\ ((id\ 1)\ (\mbox{\hyperlink{compute__ekf__sym_8m_abe119338ba11d7fd166333a3941bc2c4}{x}}\ 3))\ }
