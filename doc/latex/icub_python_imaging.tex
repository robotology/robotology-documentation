...
\hypertarget{icub_python_imaging}{}\label{icub_python_imaging}\index{Handling YARP images in Python@{Handling YARP images in Python}}
...
In the following example, a Num\+Py array with random contents is created, a YARP image is wrapped around it and sent to a YARP port. It is assumed that there is an instance of {\ttfamily yarpview} running with associated port name {\ttfamily "{}/view01"{}}.
...
\begin{DoxyItemize}
\item {\ttfamily yarp.\+Image\+Mono} -\/ array data type\+: {\ttfamily uint8}, shape\+: (\texorpdfstring{$<$}{<}image-\/height\texorpdfstring{$>$}{>}, \texorpdfstring{$<$}{<}image-\/width\texorpdfstring{$>$}{>}), values range\+: 0 to 255
\item {\ttfamily yarp.\+Image\+Float} -\/ array data type\+: {\ttfamily float32}, shape\+: (\texorpdfstring{$<$}{<}image-\/height\texorpdfstring{$>$}{>}, \texorpdfstring{$<$}{<}image-\/width\texorpdfstring{$>$}{>}), values range\+: 0. to 255. (Note\+: this is {\itshape different} from 0. to 1. range, assumed by default e.\+g. by {\ttfamily scipy.\+ndimage} routines!)
\item {\ttfamily yarp.\+Image\+Rgb} -\/ array data type\+: {\ttfamily uint8}, shape\+: (\texorpdfstring{$<$}{<}image-\/height\texorpdfstring{$>$}{>}, \texorpdfstring{$<$}{<}image-\/width\texorpdfstring{$>$}{>}, 3), values range\+: 0 to 255
\item {\ttfamily yarp.\+Image\+Rgb\+Float} -\/ array data type\+: {\ttfamily float32}, shape\+: (\texorpdfstring{$<$}{<}image-\/height\texorpdfstring{$>$}{>}, \texorpdfstring{$<$}{<}image-\/width\texorpdfstring{$>$}{>}, 3), values range\+: 0. to 255.
\item {\ttfamily yarp.\+Image\+Rgba} -\/ array data type\+: {\ttfamily uint8}, shape\+: (\texorpdfstring{$<$}{<}image-\/height\texorpdfstring{$>$}{>}, \texorpdfstring{$<$}{<}image-\/width\texorpdfstring{$>$}{>}, 4), values range\+: 0 to 255
\end{DoxyItemize}
...
\end{DoxyEnumerate}\hypertarget{icub_python_imaging_sec_yarp_to_python}{}\doxysection{\texorpdfstring{Receiving data from YARP to Python}{Receiving data from YARP to Python}}\label{icub_python_imaging_sec_yarp_to_python}
The idea is pretty much the same as when sending an image to the port. In order to minimize the number of copy operations, we wrap the YARP image around a Num\+Py array before reading an image from the port. The example below assumes that \doxylink{namespaceiCub}{i\+Cub} simulator is running and that the world camera is available at port {\ttfamily "{}/icub\+Sim/cam"{}} 
...
Array interface can also be used to easily detect whether the image has been, for whatever reason, re-\/allocated behind the scenes, e.\+g. by the {\ttfamily Port.\+read()} method (see section \doxysectlink{icub_python_imaging_sec_yarp_to_python}{Receiving data from YARP to Python}{1}). In the example below it is assumed that {\ttfamily yarp\+\_\+image} has been associated with {\ttfamily img\+\_\+array} via {\ttfamily set\+External()} before {\ttfamily Port.\+read()} is called\+:
...
\end{DoxyCode}
\hypertarget{icub_python_imaging_final_example}{}\doxysection{\texorpdfstring{Real-\/world example}{Real-world example}}\label{icub_python_imaging_final_example}
Code used in this tutorial can be found at \doxylink{python__imaging_8py}{python/python\+\_\+imaging.\+py} In addition, the file contains a class implementing a simple image filter, which retrieves images through one YARP port, computes a Sobel operator, and sends the result to another port. To run this example, make sure you have the \doxylink{namespaceiCub}{i\+Cub} simulator and yarpview instance running as in examples above, and simply execute the {\ttfamily python\+\_\+image.\+py} module as a regular python program.

This file can be edited at \doxylink{python-imaging_8dox}{doc/python-\/imaging.\+dox} 