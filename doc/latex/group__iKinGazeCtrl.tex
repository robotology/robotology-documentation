\doxysubsubsection{i\+Kin\+Gaze\+Ctrl}
\hypertarget{group__iKinGazeCtrl}{}\label{group__iKinGazeCtrl}\index{iKinGazeCtrl@{iKinGazeCtrl}}
...
This module provides a controller for the \doxylink{namespaceiCub}{i\+Cub} gaze capable of steering the neck and the eyes independently performing saccades, pursuit, vergence, OCR (oculo-\/collic reflex) and VOR (vestibulo-\/ocular reflex relying on inertial data).
...
Moreover, this module also implements the server part of the \href{https://www.yarp.it/latest/classyarp_1_1dev_1_1IGazeControl.html}{\texttt{ Gaze Control Interface}}. For a tutorial on how to use the interface, please go \doxysectlink{icub_gaze_interface}{here}{0}.

\begin{DoxyNote}{Note}
If the torso is not detected alive then the module will try to keep on working with just the head part.

{\bfseries{If you\textquotesingle{}re going to use this controller for your work, please quote it within any resulting publication}}\+: Roncone A., Pattacini U., Metta G. \& Natale L., "{}\+A Cartesian 6-\/\+Do\+F Gaze Controller for Humanoid Robots"{}, {\itshape Proceedings of Robotics\+: Science and Systems}, Ann Arbor, MI, June 18-\/22, 2016.
\end{DoxyNote}
{\bfseries{Reminder}} ~\newline
...
\begin{DoxyNote}{Note}
A video on \doxylink{namespaceiCub}{i\+Cub} gazing can be seen \href{https://youtu.be/I4ZKfAvs1y0}{\texttt{ here}}.
\end{DoxyNote}
...
\begin{DoxyItemize}
\item YARP libraries.
\item \doxylink{group__iKin}{i\+Kin} library (it requires IPOPT).
...
\begin{DoxyItemize}
\item Enable/disable saccadic movements; the parameter {\itshape switch} can be therefore \mbox{[}"{}on"{}\texorpdfstring{$\vert$}{|}"{}off"{}\mbox{]}, being "{}on"{} by default.
\end{DoxyItemize}

--neck\+\_\+position\+\_\+control {\itshape switch} 
\begin{DoxyItemize}
\item Enable/disable low-\/level position control of the neck; the parameter {\itshape switch} can be therefore \mbox{[}"{}on"{}\texorpdfstring{$\vert$}{|}"{}off"{}\mbox{]}, being "{}on"{} by default.
\end{DoxyItemize}

--imu\+::mode {\itshape switch} 
\begin{DoxyItemize}
\item Enable/disable stabilization using IMU data; the parameter {\itshape switch} can be therefore \mbox{[}"{}on"{}\texorpdfstring{$\vert$}{|}"{}off"{}\mbox{]}, being "{}on"{} by default.
\end{DoxyItemize}
...
\begin{DoxyItemize}
\item Specify the contribution of the vestibulo-\/ocular reflex (VOR) in computing the final counter-\/rotation of the eyes due to neck rotation. To turn off the VOR just set the {\itshape gain} equal to 0.\+0. By default {\itshape gain} is 1.\+0, that means {\itshape "{}full   contribution"{}}. If {\itshape imu\+::mode} is "{}off"{}, then the gain is 0.\+0 by default. Values of the gain greater than 1.\+0 mean {\itshape "{}contribution amplified"{}}. --imu\+::timeout
\item Specify the read timeout (in seconds) for the imu data. The default is 0.\+04 seconds.
\end{DoxyItemize}

--ocr {\itshape gain} 
\begin{DoxyItemize}
\item Specify the contribution of the oculo-\/collic reflex (OCR) in computing the counter-\/rotation of the eyes due to neck rotation. To turn off the OCR just set the {\itshape gain} equal to 0.\+0. Default values are 0.\+0 if {\itshape imu\+::mode} is "{}off"{}, 1.\+0 otherwise.
\end{DoxyItemize}
...
\begin{DoxyItemize}
\item This option specifies the kinematic structure of the head; the value {\itshape ver} is a string (e.\+g., "{}v1.\+0"{}, "{}v2.\+0"{}), being "{}v1.\+0"{} the default.
\end{DoxyItemize}
...
\begin{DoxyItemize}
\item If "{}on"{}, at startup default values and cameras values retrieved from file will be overwritten by those values contained in the tweak file. The {\itshape switch} is "{}on"{} by default.
...
The ports the module is connected to\+: e.\+g. /icub/head/command\+:i and so on.\hypertarget{group__tutorial__ationPrimitives_portsc_sec}{}\doxysubsubsubsection{\texorpdfstring{Ports Created}{Ports Created}}\label{group__tutorial__ationPrimitives_portsc_sec}
...
\item {\itshape /} $<$ctrl\+Name$>$/mono\+:i receives the current target position expressed in one image plane. The input data format is the Bottle \mbox{[}type u v z\mbox{]}, where {\itshape type} can be "{}left"{} or "{}right"{}, {\itshape  (u,v) } is the pixel coordinates and {\itshape z} is the guessed z-\/component in the eye\textquotesingle{}s reference frame. An alternative command modality employs the vergence given in degrees in place of the z-\/component and its format is\+: \mbox{[}type u v "{}ver"{} ver\mbox{]}.
...
\begin{DoxyItemize}
\item "{}motion-\/onset"{} $<$time$>$\+: sent out at the beginning of the motion; comprise the time instant of the source when the event took place.
\item "{}motion-\/done"{} $<$time$>$\+: sent out at the end of the motion; comprise the time instant of the source when the event took place.
\item "{}motion-\/ongoing"{} $<$time$>$ $<$checkpoint$>$\+: sent out when the portion of path given in \mbox{[}0,1\mbox{]} by the checkpoint parameter has been attained; comprise the time instant of the source when the event took place as well as the checkpoint.
\item "{}saccade-\/onset"{} $<$time$>$\+: sent out at the beginning of the saccade; comprise the time instant of the source when the event took place.
\item "{}saccade-\/done"{} $<$time$>$\+: sent out at the end of the saccade; comprise the time instant of the source when the event took place.
\item "{}stabilization-\/on"{} $<$time$>$\+: notify that gaze stabilization has been turned on; comprise the time instant of the source when the event took place.
\item "{}stabilization-\/off"{} $<$time$>$\+: notify that gaze stabilization has been turned off; comprise the time instant of the source when the event took place.
\item "{}closing"{} $<$time$>$\+: sent out when the controller is being shut down; comprise the time instant of the source when the event took place.
\item "{}suspended"{} $<$time$>$\+: sent out when the controller is suspended; comprise the time instant of the source when the event took place.
\item "{}resumed"{} $<$time$>$\+: sent out when the controller is resumed; comprise the time instant of the source when the event took place.
\item "{}comm-\/timeout"{} $<$time$>$\+: sent out when the controller gets suspended because of a communication timeout; comprise the time instant of the source when the event took place.
\end{DoxyItemize}
...
\item \mbox{[}get\mbox{]} \mbox{[}2D\mbox{]} ($<$type$>$ $<$x$>$ $<$y$>$ $<$z$>$)\+: returns the 2D pixel point whose cartesian coordinates (x,y,z) are given wrt the root reference frame as the result of its projection into the image plane $<$type$>$ \mbox{[}"{}left"{}\texorpdfstring{$\vert$}{|}"{}right"{}\mbox{]}.
\item \mbox{[}get\mbox{]} \mbox{[}3D\mbox{]} \mbox{[}mono\mbox{]} ($<$type$>$ \texorpdfstring{$<$}{<} u\texorpdfstring{$>$}{>} $<$v$>$ $<$z$>$)\+: returns the 3D point whose projected pixel coordinates (u,v) in the image plane $<$type$>$ \mbox{[}"{}left"{}\texorpdfstring{$\vert$}{|}"{}right"{}\mbox{]} along with third component $<$z$>$ in the eye\textquotesingle{}s reference frame are given.
\item \mbox{[}get\mbox{]} \mbox{[}3D\mbox{]} \mbox{[}stereo\mbox{]} (\texorpdfstring{$<$}{<} ul\texorpdfstring{$>$}{>} $<$vl$>$ $<$ur$>$ $<$vr$>$)\+: returns the 3D point whose projected pixels coordinates (ul,vl) and (ur,vr) in the image planes are provided as the result of the triangulation. \begin{DoxyNote}{Note}
The triangulation is deeply affected by uncertainties in the cameras extrinsic parameters and cameras alignment.
\end{DoxyNote}

\item \mbox{[}get\mbox{]} \mbox{[}3D\mbox{]} \mbox{[}proj\mbox{]} ($<$type$>$ \texorpdfstring{$<$}{<} u\texorpdfstring{$>$}{>} $<$v$>$ \texorpdfstring{$<$}{<} a\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<} b\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<} c\texorpdfstring{$>$}{>} $<$d$>$)\+: returns the 3D point with projected pixel coordinates (u,v) in the image plane $<$type$>$ \mbox{[}"{}left"{}\texorpdfstring{$\vert$}{|}"{}right"{}\mbox{]} that results from the intersection with the plane expressed with its implicit equation ax+by+cz+d=0 in the root reference frame.
\item \mbox{[}get\mbox{]} \mbox{[}3D\mbox{]} \mbox{[}ang\mbox{]} ($<$type$>$ $<$azi$>$ $<$ele$>$ $<$ver$>$)\+: transforms angular coordinates into cartesian coordinates. The option $<$type$>$ can be \mbox{[}"{}abs"{}\texorpdfstring{$\vert$}{|}"{}rel"{}\mbox{]}.
...
\item \mbox{[}get\mbox{]} \mbox{[}info\mbox{]}\+: returns (enclosed in a list) a property-\/like bottle containing useful information, such as the "{}head\+\_\+version"{} (e.\+g. "{}1.\+0"{}, "{}2.\+0"{}, ...), the "{}min\+\_\+allowed\+\_\+vergence"{} (in degrees), a list of the available "{}events"{}, the intrinsic and extrinsic camera parameters used.
...
\item \mbox{[}set\mbox{]} \mbox{[}pid\mbox{]} ((prop0 ($<$val$>$ $<$val$>$ ...)) (prop1) ($<$val$>$ $<$val$>$ ...))\+: sets the pid values used to converge to the target with stereo input. The pid is implemented in parallel form (\doxylink{group__PIDs}{PIDs}). Aside from the usual pid parameters a further option "{}dominant\+Eye"{} is available that enables the user to chose the dominant eye employed for the monocular approach.
\item \mbox{[}set\mbox{]} \mbox{[}tweak\mbox{]} ((prop0 ($<$val$>$ $<$val$>$ ...)) (prop1) ($<$val$>$ $<$val$>$ ...))\+: sets parameters for the low-\/level controller\textquotesingle{}s configuration.
\item \mbox{[}look\mbox{]} \mbox{[}3D\mbox{]} ($<$x$>$ $<$y$>$ $<$z$>$)\+: yields gazing at target specified as 3D point. Distances are in meters.
\item \mbox{[}look\mbox{]} \mbox{[}mono\mbox{]} ($<$type$>$ \texorpdfstring{$<$}{<} u\texorpdfstring{$>$}{>} $<$v$>$ $<$z$>$)\+: yields gazing at target specified in terms of pixel coordinates (u,v) in the image plane $<$type$>$ \mbox{[}"{}left"{}\texorpdfstring{$\vert$}{|}"{}right"{}\mbox{]} along with third component $<$z$>$ in the eye\textquotesingle{}s reference frame. Distances are in meters.
\item \mbox{[}look\mbox{]} \mbox{[}mono\mbox{]} ($<$type$>$ \texorpdfstring{$<$}{<} u\texorpdfstring{$>$}{>} $<$v$>$ "{}ver"{} $<$ver$>$)\+: yields gazing at target specified in terms of pixel coordinates (u,v) in the image plane $<$type$>$ \mbox{[}"{}left"{}\texorpdfstring{$\vert$}{|}"{}right"{}\mbox{]} along with the vergence $<$ver$>$. Angles are in degrees.
\item \mbox{[}look\mbox{]} \mbox{[}stereo\mbox{]} (\texorpdfstring{$<$}{<} ul\texorpdfstring{$>$}{>} $<$vl$>$ $<$ur$>$ $<$vr$>$)\+: yields gazing at target specified in terms of projected pixels coordinates (ul,vl) and (ur,vr) in the image planes.
\item \mbox{[}look\mbox{]} \mbox{[}ang\mbox{]} ($<$type$>$ $<$azi$>$ $<$ele$>$ $<$ver$>$)\+: yields gazing at target specified in angular coordinates. The option $<$type$>$ can be \mbox{[}"{}abs"{}\texorpdfstring{$\vert$}{|}"{}rel"{}\mbox{]}. Angles are in degrees.
...
\item \mbox{[}del\mbox{]} ($<$id0$>$ $<$id1$>$ ...)\+: delete all the contexts whose ids are contained in the list.
\item \mbox{[}register\mbox{]} \mbox{[}ongoing\mbox{]} $<$checkpoint$>$\+: register the "{}motion-\/ongoing"{} event with the given checkpoint in \mbox{[}0,1\mbox{]}.
\item \mbox{[}unregister\mbox{]} \mbox{[}ongoing\mbox{]} $<$checkpoint$>$\+: unregister the "{}motion-\/ongoing"{} event with the given checkpoint in \mbox{[}0,1\mbox{]}.
\item \mbox{[}list\mbox{]} \mbox{[}ongoing\mbox{]}\+: return the list of registered "{}motion-\/ongoing"{} events.
\item \mbox{[}susp\mbox{]}\+: suspend the module.
\item \mbox{[}run\mbox{]}\+: resume the module.
\item \mbox{[}status\mbox{]}\+: returns "{}running"{} or "{}suspended"{}.
...
\end{DoxyNote}
\hypertarget{group__iKinGazeCtrl_coor_sys_sec}{}\doxysubsubsubsection{\texorpdfstring{Coordinate System}{Coordinate System}}\label{group__iKinGazeCtrl_coor_sys_sec}
Positions (meters) refer to the root reference frame attached to the waist as in the \href{https://icub-tech-iit.github.io/documentation/icub_kinematics/icub-forward-kinematics/icub-forward-kinematics/}{\texttt{ official documentation}}.\hypertarget{group__tutorial__ationPrimitives_in_files_sec}{}\doxysubsubsubsection{\texorpdfstring{Input Data Files}{Input Data Files}}\label{group__tutorial__ationPrimitives_in_files_sec}
...
A configuration file passed through {\itshape -\/-\/cameras\+::file} contains the fields required to specify the cameras intrinsic parameters along with a roto-\/translation matrix appended to the eye kinematic (see the \doxylink{classiCub_1_1iKin_1_1iKinChain_a107e361a000db942f9a452acf3c9b020}{i\+Kin\+Chain\+::set\+HN} method) in order to achieve the alignment with the optical axes compensating for possible unknown offsets.

The final roto-\/translation matrix is meaningful only as result of the calibration of the cameras extrinsic parameters that can be obtained for instance through the \doxylink{group__icub__stereoCalib}{stereo\+Calib} module.
