...
\hypertarget{icub_periodic_thread}{}\label{icub_periodic_thread}\index{The PeriodicThread Class@{The PeriodicThread Class}}
In this tutorial we show how to write a control loop using threads.

Before you read this tutorial you should at least get accustomed with motor interfaces as described in \doxysectlink{icub_motor_control_tutorial}{Getting accustomed with motor interfaces}{0} (material in \doxysectlink{icub_basic_image_processing}{Basic Image Processing}{0} might also be useful).\hypertarget{icub_tutorial_module_sec_intro}{}\doxysection{\texorpdfstring{Introduction}{Introduction}}\label{icub_tutorial_module_sec_intro}
...
In this tutorial we show how to use the yarp\+::os\+::\+Periodic\+Thread class to write a control loop with a certain periodicity.\hypertarget{icub_periodic_thread_sec_yourownthread}{}\doxysection{\texorpdfstring{Writing your own thread}{Writing your own thread}}\label{icub_periodic_thread_sec_yourownthread}
...
\DoxyCodeLine{\ \ \ \ \ \mbox{\hyperlink{classControlThread}{ControlThread}}(\textcolor{keywordtype}{double}\ period):PeriodicThread(period)}
\DoxyCodeLine{\ \ \ \ \{...\}}
\DoxyCodeLine{\};}
...
We add three methods to the class\+: run(), thread\+Init() and thread\+Release(). The run() function will be called periodically every "{}period"{} seconds. thread\+Init() is called once when the thread starts (before run is executed). Finally the thread executes thread\+Release() when closing.

We would like to write a thread that periodically check the encoders and alternates two velocity commands. This thread will control for example the head of the robot. We need a Poly\+Driver to store the robot device (see \doxysectlink{icub_motor_control_tutorial}{Getting accustomed with motor interfaces}{0}), and an instance of IEncoders and IVelocity\+Control.
...
\end{DoxyCode}
\hypertarget{icub_periodic_thread_sec_runningthethread}{}\doxysection{\texorpdfstring{Running the thread}{Running the thread}}\label{icub_periodic_thread_sec_runningthethread}
...
Full working code of this tutorial is available here\+: \doxylink{tutorial__periodic__thread_8cpp}{src/periodic\+Thread/tutorial\+\_\+periodic\+\_\+thread.\+cpp} 